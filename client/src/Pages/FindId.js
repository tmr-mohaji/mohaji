...
const FindId = () => {
    ...
    const sendEmail = async () => {
        ...
        if ((input.current.value).includes('@') == false) {
            ...
            const emailCheck = await axios.post(process.env.REACT_APP_USER_URL + "/emailCheck", {email: input.current.value});
            ...
            if (!isEmail) {
                ...
                // 이메일로 인증코드 보내기
                const result = await axios.post(process.env.REACT_APP_BASE_URL + "/email", {email: input.current.value});
